...
#' @keywords dairy cattle milk yield
#' @import ggplot2
#' @import dplyr
#' @import viridis
#' @export
#' @examples
#' days_my(data = dairyCattle::read_cattle("cattle_data.xls", drop.zero = TRUE, add = TRUE), grid = FALSE, density = FALSE, line = FALSE)

days_my <- function(data, grid = FALSE, density = FALSE, line = FALSE) {
  ...
  plot <- plot +
    ...
    geom_text(aes(y = 유량 - 1.5, label=단축명호), size = 2, vjust = 0) +
    geom_rug() +
    theme_linedraw(base_family = "NanumGothic") +
    scale_x_continuous(breaks = seq(0, 400, 50)) +
    labs(x = "Milking days", y = "Milk yield, kg")
